...
int main(int argc, char** argv){
  int PLOT=1,OPTION=0;
  int i, j, *k, xi, xj, yi, yj, N, L=70, n, *index, nrun, cont, *conta, d, *indxi, *indxj, l, D=2;
  double kappa,*theta,dij,dx,dy,*lambda,m,err,aus, *value, dist, D0=1;
 ...
 //  sprintf(filec,"eigL%03d_%1.2fD0%1.2fkappa.txt",L,D0,kappa);
  ...
  if(OPTION==1) {
    ...
    indxi=calloc(D,sizeof(int));
    indxj=calloc(D,sizeof(int));
    cont=0;
    for(i=0;i<N;i++) {
      for(j=0;j<N;j++) {
	dist=0;
	for(l=0;l<D;l++) {
	  indxi[l]=  (int) (i/pow(L,l)) % L ;
	  indxj[l]= (int) (j/pow(L,l)) % L ;
	  dist += fabs(indxi[l]-indxj[l]);
	}
	if(dist==1)
	  a[cont++]=1;
	else
